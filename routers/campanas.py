...
async def crear_campana(req: CampanaRequest):
    ...
    objetivo_api = (
        req.objetivo
        if req.objetivo.startswith("OUTCOME_")
        else OBJETIVO_MAP.get(req.objetivo, req.objetivo)
    )
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        ...
        if not r.is_success or "id" not in d:
            raise HTTPException(
                status_code=400,
                detail=_meta_error(d, "crear campaña"),
            )
        ...
        if not r.is_success or "id" not in d:
            raise HTTPException(
                status_code=400,
                detail=_meta_error(d, "crear conjunto de anuncios"),
            )
        ...
        if not r.is_success or "id" not in d:
            raise HTTPException(
                status_code=400,
                detail=_meta_error(d, "crear creativo"),
            )
        ...
        if not r.is_success or "id" not in d:
            raise HTTPException(
                status_code=400,
                detail=_meta_error(d, "crear anuncio"),
            )
