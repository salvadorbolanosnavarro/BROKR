...
async def get_comparables_eb(colonia: str, ciudad: str,
    ...
    async with httpx.AsyncClient(timeout=60) as client:
        while len(comparables) < 50 and page <= 160:  # up to 8000 properties
            r = await client.get(
                f"{EB_BASE}/properties",
                headers=eb_headers(),
                params={"limit": 50, "page": page}
            ...
            for p in props:
                ...
                # Tipo filter — exact first, similar as fallback
                prop_type = p.get("property_type", "")
                if not any(t.lower() in prop_type.lower() for t in tipo_labels):
                    continue
                # Tag whether it's exact tipo match or similar
                is_exact_tipo = tipo_labels[0].lower() in prop_type.lower()
                ...
                # Status field is empty in this EB plan — no filter needed
